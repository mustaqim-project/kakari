...
class HomeController extends Controller
{
    ...
    public function index()
    {
        // Get the currently active live streaming if any
        $liveStream = Streaming::latest()->first(); // atau bisa pakai ->orderBy('id', 'desc')

        // Get upcoming schedule/agenda

        // Get latest articles/news
        ...
        // Get featured kajian (for the one below articles)
        $featuredKajian = News::with('category')
            ->where('status', 1)
            ->where('is_featured', 1)
        ...
        $latestPodcasts = Podcast::with('playlist')
            ->orderBy('created_at', 'desc')
            ->take(2)
            ->get();

        // Get video playlists for tabbed content
        ...
        // Get latest E-Koran
        $latestEkoran = Ekoran::where('status', 1)
            ->orderBy('created_at', 'desc')
            ->first();

        // Get pendidikan info
        $pendidikanItems = Pendidikan::where('status', 1)
            ->take(2)
            ->get();
